...
benchmark_data['threads'] = benchmark_data['name'].str.extract('Run/(\d+)_threads-\d+').apply(pd.to_numeric)
benchmark_data['cpu_cores'] = benchmark_data['name'].str.extract('Run/\d+_threads-(\d+)').apply(pd.to_numeric)
